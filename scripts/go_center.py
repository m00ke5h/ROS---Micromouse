...
class gtg_controller:
    def __init__(self):
        rospy.init_node("micromouse_center")
        ...
        rospy.Subscriber(
            "/dest", dest, self.goal_pose_callback, queue_size=1, buff_size=40
        )
        ...
        self.max_thresh = (
            0.05  # min distance upto which the speed given to the bot is 0.4
        )
    ...
    def odom_callback(self, msg):
        ...
        (
            self.current_orientation_euler[1],
            self.current_orientation_euler[0],
            self.current_orientation_euler[2],
        ) = euler_from_quaternion(
            [
                self.current_orientation_quaternion[0],
    ...
    def goal_theota(self):
        self.goal_yaw = atan2(
            self.goal[1] - self.current_pose[1], self.goal[0] - self.current_pose[0]
        )
    ...
    def control(self):
        ...
        if self.goal[0] - self.current_pose[0] >= 0:
            ...
            if self.goal[0] - self.current_pose[0] >= self.max_thresh:
                self.vel_x.linear.x = (
                    self.max_speed_x
                )  # 0.1 may change, or to cover the path faster we can put constant 0.4 and when it reaches goal we can make it 0
                # print("away from goal in X_111111111111111111")
            elif (self.goal[0] - self.current_pose[0]) <= self.max_thresh and (
                self.goal[0] - self.current_pose[0]
            ) >= self.min_thresh:
                self.vel_x.linear.x = self.max_speed_scaled_x * (
                    self.goal[0] - self.current_pose[0]
                )
            ...
            elif (self.goal[0] - self.current_pose[0]) >= (-1 * self.max_thresh) and (
                self.goal[0] - self.current_pose[0]
            ) <= (-1 * self.min_thresh):
                self.vel_x.linear.x = self.max_speed_scaled_x * (
                    self.goal[0] - self.current_pose[0]
        ...
        if self.goal[1] - self.current_pose[1] >= 0:
            ...
            elif (self.goal[1] - self.current_pose[1]) <= self.max_thresh and (
                self.goal[1] - self.current_pose[1]
            ) >= self.min_thresh:
                self.vel_y.linear.x = -self.max_speed_scaled_y * (
                    (self.goal[1] - self.current_pose[1])
                )
            ...
            elif (self.goal[1] - self.current_pose[1]) >= (-1 * self.max_thresh) and (
                self.goal[1] - self.current_pose[1]
            ) <= (-1 * self.min_thresh):
                self.vel_y.linear.x = -self.max_speed_scaled_y * (
                    (self.goal[1] - self.current_pose[1])
        ...
        if round(abs(self.current_pose[2]), 3) != 0.0000:
            self.vel_x.angular.z = 2.5 * (-self.current_pose[2]) + 0.0000001 * (
                (-self.current_pose[2] + self.previous_theota) / self.sample_time
            )
