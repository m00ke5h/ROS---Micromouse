...
class Maze:
    ...
    def updateMazeConfidence(self, x, y):
        ...
        if (fx < self.lower_thres or fx > self.upper_thres) and (
            fy < self.lower_thres or fy > self.upper_thres
        ):
            return

        elif (
            (fx < self.lower_thres and ix == 0)
            or (fx > self.upper_thres and ix == self.N - 1)
            or (fy < self.lower_thres and iy == 0)
            or (fy > self.upper_thres and iy == self.N - 1)
        ):
            return
...
if __name__ == "__main__":
    try:
        rospy.init_node("maze_algorithms")  # , disable_signals=True)
        maze_obj = Maze()
